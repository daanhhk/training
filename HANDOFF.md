...
- **Fase 3 deel 3 (uitgesteld):** §2e conditie-balans balans/driehoek-picker; her-skin legacy `.metric`-tegels (`vorm-huidig`/`vorm-stats`) naar de 3a-skin; strip-kleuren finaliseren (Optimaal nu `--accent`/oranje; Grijs-band `--bg-elevated` mogelijk te subtiel).
- **Garmin-push** nog week-niveau (geen per-dag `GarminSync`-knop) → Fase 6.

## Volgende stap
1. Verifieer Fase 3 deel 1+2 (LevelCard/MetricRow/chart-tokens/gridline-fix/niveau-venster-toggle) visueel op /dev (incognito). 2. Daarna: Fase 3 deel 3 (conditie-balans-picker + legacy-tegels re-skin + strip-kleuren) of Trainingen-bibliotheek (Fase 4).
...
- **Fase 3 deel 2 — chart-gridlines fix + niveau-venster-toggle (0301fa8 fix / b72db9c feat):** Gridlines bleven licht omdat alleen `vAxis` getokeniseerd was (hAxis = Google-default ~`#CCC`) én `--chart-grid` een `color-mix(white)` was (alpha door Charts gedropt → wit). Fix: `--chart-grid` → `var(--border-subtle)` (concreet, beide token-bestanden) + `cssColor_('var(--chart-grid/axis)')` op hAxis+vAxis gridlines+baseline van béíde charts. §2c: `renderNiveauHead_` (value + window-delta, `--good/--bad`) + `.seg` segmented `1M/6M/12M/Alles` (`niveauWindow`, default Alles) → `niveauReeksWindow_` slicet `vorm.niveauReeks` client-side → `drawNiveauChart`-redraw; mount `#niveau-head`. Losgekoppeld van `#period-toggle`/`statPeriod` (volume). Geen server-calc → geen gate-case. `--chart-line` = `--accent` (oranje).
