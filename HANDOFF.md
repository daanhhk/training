...
- **DONKERE SKIN IS LIVE.** 4 tabs. **Schema-tab compleet** (plan-kaart 1a, WeekLoad 3b, dag-detail skin 3a, web-RPE + skip-dispositie 3c). **Vorm-tab compleet**: ReadinessCard (1b) + check-in (2) + LevelCard/MetricRow/chart-tokens (3-d1) + niveau-grafiek (3-d2) + Conditie Balans-meter (3-d3) + swipe-deck (ReadinessCard⇄LevelCard) + LevelCard tier-chip/blok-%/reconciled-delta (3-d4). Volgende = Trainingen (Fase 4) of Instellingen-drawer (Fase 5).
...
## Volgende stap
1. Verifieer **Vorm-tab compleet** (deel 1–4: LevelCard/MetricRow/niveau-grafiek/Balans-meter/swipe-deck+dots/tier-chip) visueel op /dev (incognito + swipe). 2. **Fase 3 (Vorm) klaar** → volgende = Trainingen-bibliotheek (Fase 4) of Instellingen-drawer (Fase 5).
