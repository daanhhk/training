...
var ARCHETYPES = [
  ...
  // ════════════ UITBREIDING — 12 klim/FTP-kwaliteit-archetypes ════════════
  // Veld-conventies gespiegeld van de bestaande records van dezelfde intent (vo2_long /
  // threshold_long / sweetspot_long): warmup/cooldown/fill, int-min vs int-sec (vo2_microburst),
  // over-under/piramide als steady-sequentie. duurRange.min = warmup+core+cooldown (geconvention,
  // getest via 'lib <id> min~core'); .max = + Z2-fill-ruimte.

  // ── VO2 (zone 5) ──
  { id: 'vo2_4x5', structuurtype: 'intervals', effectTags: ['vo2'], zone: 5,
    duurRange: [61, 100],
    warmup: { durMin: 15, pctLo: 55, pctHi: 80 },
    core: [{ kind: 'int', label: 'VO2 4×5', reps: 4, onMin: 5, onPctLo: 106, onPctHi: 112, offMin: 4, offPct: 50 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'VO2max 4×5', focus: 'vo2 capacity',
    eindopmerking: 'Langere VO2-blokken — iets lager, langer aanhouden.' },
  { id: 'vo2_40_20', structuurtype: 'microburst', effectTags: ['vo2'], zone: 5,
    duurRange: [37, 75],
    warmup: { durMin: 15, pctLo: 55, pctHi: 80 },
    core: [{ kind: 'int', label: 'VO2 40/20', reps: 12, onSec: 40, onPctLo: 120, onPctHi: 125, offSec: 20, offPct: 55 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'VO2max 40/20', focus: 'vo2 microburst',
    eindopmerking: 'Rønnestad-microbursts — hoog vermogen, korte pauzes.' },
  { id: 'vo2_30_15_sets', structuurtype: 'microburst', effectTags: ['vo2'], zone: 5,
    duurRange: [62, 85],
    warmup: { durMin: 15, pctLo: 55, pctHi: 80 },
    core: [
      { kind: 'int', label: '30/15 set 1', reps: 13, onSec: 30, onPctLo: 118, onPctHi: 125, offSec: 15, offPct: 55 },
      { kind: 'steady', label: 'Herstel', durMin: 3, pct: 55, note: 'Tussen de sets' },
      { kind: 'int', label: '30/15 set 2', reps: 13, onSec: 30, onPctLo: 118, onPctHi: 125, offSec: 15, offPct: 55 },
      { kind: 'steady', label: 'Herstel', durMin: 3, pct: 55, note: 'Tussen de sets' },
      { kind: 'int', label: '30/15 set 3', reps: 13, onSec: 30, onPctLo: 118, onPctHi: 125, offSec: 15, offPct: 55 }
    ],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'VO2max 30/15', focus: 'vo2 microburst sets',
    eindopmerking: 'Drie sets 30/15 — klassieke VO2-stapeling.' },
  { id: 'vo2_sandwich', structuurtype: 'sandwich', effectTags: ['vo2'], zone: 5,
    duurRange: [61, 95],
    warmup: { durMin: 15, pctLo: 55, pctHi: 80 },
    core: [
      { kind: 'steady', label: 'Tempo', durMin: 10, pct: 90, note: 'Aanloop op tempo' },
      { kind: 'int', label: 'VO2 surges', reps: 4, onMin: 2, onPctLo: 110, onPctHi: 114, offMin: 2, offPct: 60 },
      { kind: 'steady', label: 'Tempo', durMin: 10, pct: 90, note: 'Uitlopen op tempo' }
    ],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'VO2 sandwich', focus: 'climb surge',
    eindopmerking: 'Tempo, vier VO2-surges, tempo — als een klim met versnellingen.' },

  // ── DREMPEL (zone 4) ──
  { id: 'threshold_2x20', structuurtype: 'intervals', effectTags: ['drempel'], zone: 4,
    duurRange: [75, 110],
    warmup: { durMin: 15, pctLo: 55, pctHi: 75 },
    core: [{ kind: 'int', label: 'Drempel', reps: 2, onMin: 20, onPctLo: 95, onPctHi: 100, offMin: 5, offPct: 55 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
  ...
  { id: 'threshold_4x10', structuurtype: 'intervals', effectTags: ['drempel'], zone: 4,
    duurRange: [77, 110],
    warmup: { durMin: 15, pctLo: 55, pctHi: 75 },
    core: [{ kind: 'int', label: 'Drempel', reps: 4, onMin: 10, onPctLo: 96, onPctHi: 102, offMin: 3, offPct: 55 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Drempel 4×10', focus: 'threshold',
    eindopmerking: 'Kortere drempelreps, meer totaal aan de grens.' },
  { id: 'threshold_overunder_long', structuurtype: 'intervals', effectTags: ['drempel'], zone: 4,
    duurRange: [69, 105],
    warmup: { durMin: 15, pctLo: 55, pctHi: 75 },
    core: [
      ...
      { kind: 'steady', label: 'Under', durMin: 2, pct: 93, note: 'Onder FTP' },
      { kind: 'steady', label: 'Over', durMin: 2, pct: 103, note: 'Boven FTP' },
      { kind: 'steady', label: 'Under', durMin: 2, pct: 93, note: 'Onder, afsluiten' }
    ],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Drempel over-under lang', focus: 'over-under',
    eindopmerking: 'Lange over-unders — surge-en-settle rond je drempel.' },
  { id: 'threshold_pyramid', structuurtype: 'pyramid', effectTags: ['drempel'], zone: 4,
    duurRange: [75, 110],
    warmup: { durMin: 15, pctLo: 55, pctHi: 75 },
    core: [
      { kind: 'steady', label: 'Drempel 5', durMin: 5, pct: 96, note: 'Opbouwen' },
      { kind: 'steady', label: 'Herstel', durMin: 3, pct: 55, note: 'Kort lossen' },
      { kind: 'steady', label: 'Drempel 8', durMin: 8, pct: 98, note: 'Door' },
      { kind: 'steady', label: 'Herstel', durMin: 3, pct: 55, note: 'Kort lossen' },
      { kind: 'steady', label: 'Drempel 12', durMin: 12, pct: 100, note: 'Piekblok' },
      { kind: 'steady', label: 'Herstel', durMin: 3, pct: 55, note: 'Kort lossen' },
      { kind: 'steady', label: 'Drempel 8', durMin: 8, pct: 98, note: 'Afbouwen' },
      { kind: 'steady', label: 'Herstel', durMin: 3, pct: 55, note: 'Kort lossen' },
      { kind: 'steady', label: 'Drempel 5', durMin: 5, pct: 96, note: 'Afsluiten' }
    ],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Drempel piramide', focus: 'threshold pyramid',
    eindopmerking: 'Oplopend dan aflopend — drempel met ritme.' },

  // ── SWEET SPOT (zone 4) ──
  { id: 'sweetspot_3x15', structuurtype: 'intervals', effectTags: ['sweetspot'], zone: 4,
    duurRange: [82, 115],
    warmup: { durMin: 15, pctLo: 55, pctHi: 70 },
    core: [{ kind: 'int', label: 'Sweet Spot', reps: 3, onMin: 15, onPctLo: 88, onPctHi: 93, offMin: 4, offPct: 55 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Sweet spot 3×15', focus: 'sweetspot',
    eindopmerking: 'Drie sweet-spot-blokken — degelijk volume.' },
  { id: 'sweetspot_4x12', structuurtype: 'intervals', effectTags: ['sweetspot'], zone: 4,
    duurRange: [85, 120],
    warmup: { durMin: 15, pctLo: 55, pctHi: 70 },
    core: [{ kind: 'int', label: 'Sweet Spot', reps: 4, onMin: 12, onPctLo: 89, onPctHi: 92, offMin: 3, offPct: 55 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Sweet spot 4×12', focus: 'sweetspot',
    eindopmerking: 'Vier blokken — meer totaal in sweet spot.' },
  { id: 'sweetspot_overunder', structuurtype: 'intervals', effectTags: ['sweetspot'], zone: 4,
    duurRange: [69, 105],
    warmup: { durMin: 15, pctLo: 55, pctHi: 70 },
    core: [
      { kind: 'steady', label: 'Over', durMin: 3, pct: 94, note: 'Net boven sweet spot' },
      ...
      { kind: 'steady', label: 'Under', durMin: 3, pct: 86, note: 'Net onder' },
      { kind: 'steady', label: 'Over', durMin: 3, pct: 94, note: 'Net boven sweet spot' },
      { kind: 'steady', label: 'Under', durMin: 3, pct: 86, note: 'Onder, afsluiten' }
    ],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Sweet spot over-under', focus: 'over-under',
    eindopmerking: 'Sweet-spot met golfbeweging — net over en onder.' },
  { id: 'sweetspot_long_climb', structuurtype: 'intervals', effectTags: ['sweetspot'], zone: 4,
    duurRange: [85, 125],
    warmup: { durMin: 15, pctLo: 55, pctHi: 70 },
    core: [{ kind: 'int', label: 'Sweet Spot klim', reps: 2, onMin: 25, onPctLo: 89, onPctHi: 93, offMin: 5, offPct: 55 }],
    cooldown: { durMin: 10, pctLo: 45, pctHi: 55 },
    fill: { zone: 2, pct: 65 },
    naam: 'Sweet spot klim', focus: 'long climb',
